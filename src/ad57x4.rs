...
use crate::{
    Ad57xxShared, CommandByte, Config, Data, Error, Function, OutputRange,
    PowerConfig,
    common::Ad57xx
};
...
impl<DEV, E> Ad57xxShared<DEV> where
DEV: SpiDevice<Error = E>{
    /// Create a new quad channel AD57xx DAC
...
impl<DEV, E> Ad57xx<DEV, E> for Ad57xxShared<DEV>
where
    DEV: SpiDevice<Error = E>,
{
    type CH = Channel;
    ...
    /// After power up a timeout of 10us is required before loading the corresponding DAC register
     fn set_power(&mut self, chan: Self::CH, pwr: bool) -> Result<(), Error<E>> {
        match chan {
    ...
    /// ```
    ///
     fn set_dac_output(&mut self, chan: Channel, val: u16) -> Result<(), Error<E>> {
    ...
    /// Set the output range of the selected DAC channel
     fn set_output_range(&mut self, chan: Channel, range: OutputRange) -> Result<(), Error<E>> {
